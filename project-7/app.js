...
form.addEventListener("submit", (e) => {
  ...
  if (editing) {
    ...
    // send email reminder for the new task
    sendEmail(newTask);
  }
...
async function sendEmail(task) {
  ...
  showTaskAlert(task, "success", "Email reminder sent!");
}

function showTaskAlert(task, type = "success", message = "Task added!") {
  const alertMessage = `
${message}
Task Title: ${task.title}
Task Priority: ${task.priority}
Task Note: ${task.notes || "No notes"}
Due: ${task.due || "None"}
  `;
